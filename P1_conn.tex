...
by $\calH(k)$. See %\cite[Definition 3.3.1]{Hhorn}
...
cofibrations.
%i.e., $\bbA^1$-local weak equivalences which are also
%cofibrations.
...
% \marginpar{
%   Should I include the explicit model of the $\bbA^1$ localized
%   unstable homotopy category? It should be described in the other
%   lectures. Perhaps I should at least set notation.
% }
...
\begin{notation}
  % For $\calX, \calY \in \Spc_{\bullet}(k)$, we write
...
\end{proposition}
\marginpar{Need this result for spectra for the $\bbG_m$ loop space
  calculation.}
\begin{proof}
...
%\footnote{I may be able to prove this
...
\begin{proof}
  ...
  pointed internal hom for this. \marginpar{give ref}
\end{proof}
...
%Let $E \in \SH(k)_{\geq 0}$ and $F \in \SH(k)_{\leq 0}$.
...
\begin{thebibliography}{OOOO}
    ...
    \bibitem[Ayo08]{Ayoub} Ayoub, Joseph, {\it Un contre-exemple {\'a}
  ...
  online preprint (1991).
    ...
    \bibitem[H-Spt]{H-Spt} Hovey, Mark, {\it Spectra and symmetric
    spectra in general model categories}. journal? (2001).

    \bibitem[HSS]{HSS} Hovey, Mark; Shipley, Brooke; Smith, Jeff
