...
\begin{remark}
  We will also show that for any pointed space $\calY$, the $\bbP^1$
  suspension spectrum $\Sigma^{\infty} \calY$ is $-1$ connected, i.e.,
  the homotopy sheaves $\pi^{\bbA^1}_{n+\alpha m}(\calY)$ vanish for
  $n \leq -1$. This will allow us to establish a $t$-structure on
  $\SH(k)$.
...
Give the unstable model category the injective model structure to
start. (This is Morel's choice, so we stick with it)

Weak equivalences: $\calX \to \calY$ iff for all $U$ the map
$\calX(U)\to \calY(U)$ a simplicial w.e.

Cofibs: $\calX \to \calY$ is a cofib. iff for any $U\in \Sm/k$ the map
$\calX(U) \to \calY(U)$ is a monomorphism.

Fibrations: what they need to be.

The ass. htpy. cat. to the injective model category structure is
denoted $\calH_s(k)$. The pointed htpy. cat. is denoted
$\calH_{s,\bullet}(k)$.

Important properties/constructions in these categories: Cartesian
closed, i.e., have smash/tensor $\times$ and $\wedge$, with right
adjoints $\underline{Hom}$ and
$\underline{Hom}_{\bullet}$. Representable sheaf functor and constant
...
The general theory of Bousfield localization then gives a
...
Morel gives an explicit construction which takes a pointed space
$\calY$ and produces a space $L^{\infty}\calY$ which is $\bbA^1$ local
and a map $\calY \to L^{\infty}\calY$ which is an $\bbA^1$ weak
equivalence.
...
\begin{proposition}
  Let $\calX$ be a pointed space. Then if for any finitely generated
  field $F$ over $k$, $\pi_0(\calX)(F) = 0$, then the sheaf
  $\pi_0(\calX)$ is trivial. 
\end{proposition}
...
\begin{enumerate}
  ...
  then follows that the cone of $E_{\geq 0} \to E$ satisfies the
  properties needed, and we make take this as the definition of
  $E_{<0}$, or $E_{\leq -1}$. The uniqueness follows by general
  properties of triangulated categories. 
\end{enumerate}
\end{proof}
