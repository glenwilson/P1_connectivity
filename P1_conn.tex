...
\begin{proof}
  ...
  To establish the second point, we calculate 
  \begin{align*}
  ...
  We now know that at least at $\Spec k$, the spectra
  $\underline{\Hom}(\bbG_m, HM)$ and $H(M_{-1})$ agree.

  We now need to jazz up the proof to an equivalence of sheaves of
  spectra by using base change arguments. How to do this carefully?
\end{proof}
...
\begin{proof}
  ...
  $\underline{\Hom}(\bbG_m,HM)\to H(M_{-1})$ is a weak equivalence. 
...
% \begin{lemma}
%   If $M$ is a strictly $\bbA^1$ invariant sheaf of abelian groups,
%   then the canonical map
%   \begin{equation*}
%     \underline{Hom}(\bbG_m, HM) \to H(M_{-1})
%   \end{equation*}
